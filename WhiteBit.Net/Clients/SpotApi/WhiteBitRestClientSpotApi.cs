...
{
    /// <inheritdoc cref="IWhiteBitRestClientSpotApi" />
    public class WhiteBitRestClientSpotApi : RestApiClient, IWhiteBitRestClientSpotApi, ISpotClient
    {
        #region fields 
        /// <inheritdoc />
        ...
        internal WhiteBitRestClientSpotApi(ILogger logger, HttpClient? httpClient, WhiteBitRestOptions options)
            : base(logger, httpClient, options.Environment.SpotRestAddress, options, options.SpotOptions)
        {
            Account = new WhiteBitRestClientSpotApiAccount(this);
            ExchangeData = new WhiteBitRestClientSpotApiExchangeData(logger, this);
            Trading = new WhiteBitRestClientSpotApiTrading(logger, this);
        ...
        internal async Task<WebCallResult<WhiteBitPlacedOrder>> PlaceOrderInternal(Uri uri,
            string symbol,
            ...
            CancellationToken ct = default)
        {
            ...
            if (!rulesCheck.Passed)
            {
                ...
                return new WebCallResult<WhiteBitPlacedOrder>(new ArgumentError(rulesCheck.ErrorMessage!));
            }
            ...
            return await SendRequestInternal<WhiteBitPlacedOrder>(uri, HttpMethod.Post, ct, parameters, true, weight: weight).ConfigureAwait(false);
        }
        ...
        internal async Task<WhiteBitTradeRuleResult> CheckTradeRules(string symbol, decimal? quantity, decimal? quoteQuantity, decimal? price, decimal? stopPrice, SpotOrderType? type, CancellationToken ct)
        {
        ...
        async Task<WebCallResult<OrderId>> ISpotClient.PlaceOrderAsync(string symbol, CommonOrderSide side, CommonOrderType type, decimal quantity, decimal? price, string? accountId, string? clientOrderId, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.PlaceOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<Order>> IBaseRestClient.GetOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<IEnumerable<UserTrade>>> IBaseRestClient.GetOrderTradesAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetOrderTradesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetClosedOrdersAsync), nameof(symbol));
        ...
        async Task<WebCallResult<OrderId>> IBaseRestClient.CancelOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.CancelOrderAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetTickerAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetKlinesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetOrderBookAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(ISpotClient.GetRecentTradesAsync), nameof(symbol));
