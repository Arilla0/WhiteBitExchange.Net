...
{
    /// <inheritdoc cref="IWhiteBitSocketClientCoinFuturesApi" />
    public class WhiteBitSocketClientCoinFuturesApi : SocketApiClient, IWhiteBitSocketClientCoinFuturesApi
    {
        #region fields
        ...
        internal WhiteBitSocketClientCoinFuturesApi(ILogger logger, WhiteBitSocketOptions options) :
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToKlineUpdatesAsync(string symbol, KlineInterval interval, Action<DataEvent<IWhiteBitStreamKlineData>> onMessage, CancellationToken ct = default) => await SubscribeToKlineUpdatesAsync(new[] { symbol }, new[] { interval }, onMessage, ct).ConfigureAwait(false);
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToKlineUpdatesAsync(IEnumerable<string> symbols, IEnumerable<KlineInterval> intervals, Action<DataEvent<IWhiteBitStreamKlineData>> onMessage, CancellationToken ct = default)
        {
            ...
            var handler = new Action<DataEvent<WhiteBitCombinedStream<WhiteBitFuturesStreamCoinKlineData>>>(data =>
            {
                var result = data.Data.Data;
                onMessage(data.As<IWhiteBitStreamKlineData>(result, result.Symbol));
            });
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToIndexPriceUpdatesAsync(string pair, int? updateInterval, Action<DataEvent<IEnumerable<WhiteBitFuturesStreamIndexPrice>>> onMessage, CancellationToken ct = default) => await SubscribeToIndexPriceUpdatesAsync(new[] { pair }, updateInterval, onMessage, ct).ConfigureAwait(false);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToIndexPriceUpdatesAsync(IEnumerable<string> pairs, int? updateInterval, Action<DataEvent<IEnumerable<WhiteBitFuturesStreamIndexPrice>>> onMessage, CancellationToken ct = default)
        {
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToMarkPriceUpdatesAsync(string symbol, int? updateInterval, Action<DataEvent<IEnumerable<WhiteBitFuturesCoinStreamMarkPrice>>> onMessage, CancellationToken ct = default) => await SubscribeToMarkPriceUpdatesAsync(new[] { symbol }, updateInterval, onMessage, ct).ConfigureAwait(false);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToMarkPriceUpdatesAsync(IEnumerable<string> symbols, int? updateInterval, Action<DataEvent<IEnumerable<WhiteBitFuturesCoinStreamMarkPrice>>> onMessage, CancellationToken ct = default)
        {
            symbols.ValidateNotNull(nameof(symbols));
            updateInterval?.ValidateIntValues(nameof(updateInterval), 1000, 3000);
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToContinuousContractKlineUpdatesAsync(string pair, ContractType contractType, KlineInterval interval, Action<DataEvent<WhiteBitStreamKlineData>> onMessage, CancellationToken ct = default) => await SubscribeToContinuousContractKlineUpdatesAsync(new[] { pair }, contractType, interval, onMessage, ct).ConfigureAwait(false);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToContinuousContractKlineUpdatesAsync(IEnumerable<string> pairs, ContractType contractType, KlineInterval interval, Action<DataEvent<WhiteBitStreamKlineData>> onMessage, CancellationToken ct = default)
        {
            pairs.ValidateNotNull(nameof(pairs));
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToIndexKlineUpdatesAsync(string pair, KlineInterval interval, Action<DataEvent<WhiteBitStreamIndexKlineData>> onMessage, CancellationToken ct = default) => await SubscribeToIndexKlineUpdatesAsync(new[] { pair }, interval, onMessage, ct).ConfigureAwait(false);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToIndexKlineUpdatesAsync(IEnumerable<string> pairs, KlineInterval interval, Action<DataEvent<WhiteBitStreamIndexKlineData>> onMessage, CancellationToken ct = default)
        {
            pairs.ValidateNotNull(nameof(pairs));
            var handler = new Action<DataEvent<WhiteBitCombinedStream<WhiteBitStreamIndexKlineData>>>(data => onMessage(data.As(data.Data.Data, data.Data.Data.Symbol)));
            pairs = pairs.Select(a => a.ToLower(CultureInfo.InvariantCulture) +
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToMarkPriceKlineUpdatesAsync(string symbol, KlineInterval interval, Action<DataEvent<WhiteBitStreamIndexKlineData>> onMessage, CancellationToken ct = default) => await SubscribeToMarkPriceKlineUpdatesAsync(new[] { symbol }, interval, onMessage, ct).ConfigureAwait(false);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToMarkPriceKlineUpdatesAsync(IEnumerable<string> symbols, KlineInterval interval, Action<DataEvent<WhiteBitStreamIndexKlineData>> onMessage, CancellationToken ct = default)
        {
            symbols.ValidateNotNull(nameof(symbols));
            var handler = new Action<DataEvent<WhiteBitCombinedStream<WhiteBitStreamIndexKlineData>>>(data => onMessage(data.As(data.Data.Data, data.Data.Data.Symbol)));
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToMiniTickerUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<IWhiteBitMiniTick>> onMessage, CancellationToken ct = default)
        {
            ...
            var handler = new Action<DataEvent<WhiteBitCombinedStream<WhiteBitStreamCoinMiniTick>>>(data => onMessage(data.As<IWhiteBitMiniTick>(data.Data.Data, data.Data.Data.Symbol)));
        ...
        {
            var handler = new Action<DataEvent<WhiteBitCombinedStream<IEnumerable<WhiteBitStreamCoinMiniTick>>>>(data => onMessage(data.As<IEnumerable<IWhiteBitMiniTick>>(data.Data.Data, data.Data.Stream)));
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToTickerUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<IWhiteBit24HPrice>> onMessage, CancellationToken ct = default)
        {
            ...
            var handler = new Action<DataEvent<WhiteBitCombinedStream<WhiteBitStreamCoinTick>>>(data => onMessage(data.As<IWhiteBit24HPrice>(data.Data.Data, data.Data.Data.Symbol)));
        ...
        {
            var handler = new Action<DataEvent<WhiteBitCombinedStream<IEnumerable<WhiteBitStreamCoinTick>>>>(data => onMessage(data.As<IEnumerable<IWhiteBit24HPrice>>(data.Data.Data, data.Data.Stream)));
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToAllMarkPriceUpdatesAsync(Action<DataEvent<IEnumerable<WhiteBitFuturesCoinStreamMarkPrice>>> onMessage, int? updateInterval = null, CancellationToken ct = default)
        {
            updateInterval?.ValidateIntValues(nameof(updateInterval), 1000, 3000);

            var handler = new Action<DataEvent<WhiteBitCombinedStream<IEnumerable<WhiteBitFuturesCoinStreamMarkPrice>>>>(data => onMessage(data.As(data.Data.Data, data.Data.Stream)));
        ...
        private void HandlePossibleSingleData<T>(DataEvent<JToken> data, Action<DataEvent<IEnumerable<T>>> onMessage)
        {
            ...
            if (internalData.Type == JTokenType.Array)
            {
                ...
                var deserialized = Deserialize<WhiteBitCombinedStream<IEnumerable<T>>>(data.Data);
                if (!deserialized)
                    return;
            ...
            else
            {
                ...
                var deserialized = Deserialize<WhiteBitCombinedStream<T>>(
                        data.Data);
                if (!deserialized)
                    return;
        ...
        protected override async Task<bool> UnsubscribeAsync(SocketConnection connection, SocketSubscription subscription)
        {
            ...
            var unsub = new WhiteBitSocketRequest { Method = "UNSUBSCRIBE", Params = topics.ToArray(), Id = ExchangeHelpers.NextId() };
            var result = false;
