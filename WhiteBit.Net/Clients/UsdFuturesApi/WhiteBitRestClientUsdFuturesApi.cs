...
{
    /// <inheritdoc cref="IWhiteBitRestClientUsdFuturesApi" />
    public class WhiteBitRestClientUsdFuturesApi : RestApiClient, IWhiteBitRestClientUsdFuturesApi, IFuturesClient
    {
        #region fields 
        /// <inheritdoc />
        ...
        internal WhiteBitFuturesUsdtExchangeInfo? _exchangeInfo;
        ...
        internal WhiteBitRestClientUsdFuturesApi(ILogger logger, HttpClient? httpClient, WhiteBitRestOptions options)
            : base(logger, httpClient, options.Environment.UsdFuturesRestAddress!, options, options.UsdFuturesOptions)
        {
            Account = new WhiteBitRestClientUsdFuturesApiAccount(this);
            ExchangeData = new WhiteBitRestClientUsdFuturesApiExchangeData(logger, this);
            Trading = new WhiteBitRestClientUsdFuturesApiTrading(logger, this);
        ...
        internal async Task<WhiteBitTradeRuleResult> CheckTradeRules(string symbol, decimal? quantity, decimal? quoteQuantity, decimal? price, decimal? stopPrice, FuturesOrderType type, CancellationToken ct)
        {
            ...
            if (ApiOptions.TradeRulesBehaviour == TradeRulesBehaviour.None)
                return WhiteBitTradeRuleResult.CreatePassed(outputQuantity, quoteQuantity, outputPrice, outputStopPrice);
            ...
            if (!symbolData.OrderTypes.Contains(type))
                return WhiteBitTradeRuleResult.CreateFailed($"Trade rules check failed: {type} order type not allowed for {symbol}");
        ...
        async Task<WebCallResult<OrderId>> IFuturesClient.PlaceOrderAsync(string symbol, CommonOrderSide side, CommonOrderType type, decimal quantity, decimal? price, int? leverage, string? accountId, string? clientOrderId, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.PlaceOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<Order>> IBaseRestClient.GetOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<IEnumerable<UserTrade>>> IBaseRestClient.GetOrderTradesAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetOrderTradesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetClosedOrdersAsync), nameof(symbol));
        ...
        async Task<WebCallResult<OrderId>> IBaseRestClient.CancelOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.CancelOrderAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetTickerAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetKlinesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetOrderBookAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for WhiteBit " + nameof(IFuturesClient.GetRecentTradesAsync), nameof(symbol));
        ...
        private static OrderSide GetOrderSide(CommonOrderSide side)
        {
            ...
            throw new ArgumentException("Unsupported order side for WhiteBit order: " + side);
        }
        ...
        private static FuturesOrderType GetOrderType(CommonOrderType type)
        {
            ...
            throw new ArgumentException("Unsupported order type for WhiteBit order: " + type);
        }
        ...
        private static KlineInterval GetKlineIntervalFromTimespan(TimeSpan timeSpan)
        {
            ...
            throw new ArgumentException("Unsupported timespan for WhiteBit Klines, check supported intervals using WhiteBit.Net.Enums.KlineInterval");
        }
